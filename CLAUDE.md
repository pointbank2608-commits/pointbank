...
- `/curriculum` (네비 라벨 "내 수업", 2026-09-26에 "내 커리큘럼"에서 바꿈 — 사용자가 가장 많이 쓸 핵심 기능) — 캔바식 슬라이드 빌더(`LessonSlideSorter.tsx`, 이미지·유튜브·게임·수업 자료실). **준비는 편집 화면에서 끝내고, "발표하기" 뒤엔 수업만 한다(2026-09-25 사용자 결정).**
  ...
  - **수업 레시피(2026-09-27)**: 기능이 많아 막막한 선생님을 위해 "새 수업 만들기"에서 빈 수업일 때 레시피 카드(`LessonRecipePicker`)를 보여 주고, 고르면 슬라이드가 순서대로 채워진다(`lib/lessonRecipes.ts`의 `LESSON_RECIPES` — 단어 수업·파닉스·문법·노래·지문·보카 대회·온라인·10분 복습). 단어장이 필요한 레시피는 단어장이 없으면 잠김, 문법은 문법 항목을 골라 예문으로 문장 배열하기 게임 내용을 만들고(`makeUnscrambleTemplate`), 노래·지문은 원문을 받는다. 게임 슬라이드는 템플릿 없이 넣어 저장할 때 단어장으로 자동 생성. 빈 수업을 레시피로 채우면 되돌리기로 빈 수업(레시피 고르기)으로 돌아간다. 새 레시피 = 배열 한 줄 + `recipes.<id>Name/Desc`. 다음: 짧은 영상·발표 중 한 줄 안내, AI 수업 만들기가 레시피를 고르게.
  - **온라인(줌) 수업·수업 공유(2026-09-27, `supabase/030`)**:
    - 슬라이드 화면은 `LessonSlideContent`(발표 `LessonSlideViewerPage`와 학생 따라보기가 같이 씀) — 새 슬라이드 종류는 여기에 그릴 것.
    - **학생 따라보기**: 발표 진행바 "온라인"(`OnlineClassPanel`) → `lesson_views`(6자리 번호, 수업 묶음 `viewSnapshot` = 슬라이드 + 수업 단어). 학생은 `/watch/:code`(`LessonWatchPage`, 로그인 없음, `App.tsx`가 인증보다 먼저)로 `lesson_view_get`/`lesson_view_state`. 러너가 슬라이드를 넘길 때 방송(`lesson-view-<code>`) + DB 기록, 학생은 3초 폴링으로 따라잡음. 슬라이드 안 단계는 `lib/presentSync.ts`의 `useSyncedSubState` — 문법(예문 수·화면·해석)·노래(줄·해석·빈칸)·카드(순서·위치·뒤집힘)·단어 소개(순서·단어·단계)가 선생님은 알리고 학생은 받는다(학생은 키보드·자동 읽기 꺼짐). **단계 있는 새 보드를 만들면 이 훅을 붙일 것.** 게임·자료실은 학생에게 "선생님 화면을 보세요"(퀴즈쇼면 참가 버튼), 출석은 이름을 안 보여 준다. 수업을 마치면 따라보기도 끝난다.
    - **수업 공유 링크**: 수업 카드 공유 버튼(`ShareLessonModal`) → `lesson_shares`에 묶음(`lib/lessonShare.ts`: 슬라이드 + 단어장 단어 + 게임 내용, 학생 정보 없음, 노래·지문 슬라이드는 기본으로 뺌). 받은 쪽 `/share/:token`(`SharedLessonPage`, 로그인 전에도 미리보기, 로그인 전이면 `PENDING_SHARE_KEY`로 기억 → 대시보드가 다시 보냄) → 반 골라 `importSharedLesson`(단어장·게임 내용·슬라이드 id 새로). 링크는 만든 순간의 묶음(고친 뒤엔 새 링크). 전용 파일(.classbank) 내보내기는 보류(사용자와 더 생각하기로).
    - 퀴즈쇼 대기실 "링크 복사"(줌 채팅용).
