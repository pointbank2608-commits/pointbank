...
function defaultItems(): GameItem[] {
  return [1, 2, 3, 4, 5].map((n) => ({ id: uid(), label: i18n.t('gameBaskin31.defaultItem', { n }) }));
}

const DEFAULT_TARGET_COUNT = 31;

export default function Baskin31Page() {
  const { t } = useTranslation();
  const g = useGameTemplates({
    gameType: 'baskin31',
    defaultItems,
    defaultConfig: () => ({ targetCount: DEFAULT_TARGET_COUNT }),
  });
  const {
    isStaff,
    classes,
  ...
  const [targetInput, setTargetInput] = useState(String(targetCount));

  useEffect(() => {
    setTargetInput(String(targetCount));
  }, [targetCount]);
  ...
  async function clearAllItems() {
    ...
    if (!confirm(t('gameBaskin31.clearAllConfirm'))) return;
    await persistItems([]);
  }
  ...
  async function addQuickItem() {
    ...
    if (await persistItems([...selected.items, { id: uid(), label: i18n.t('gameBaskin31.defaultItem', { n }) }])) {
  ...
  const classPicker = isStaff ? (
    ...
    <h2 className="font-title-md text-title-md text-on-surface">
      {t('gameBaskin31.studentClassTitle', { className: studentClassName })}
    </h2>
  );
  ...
  const createForm = isStaff && showCreateForm && (
    <div className="bg-surface-container-lowest rounded-xl p-5 shadow-[0_4px_20px_rgba(39,101,168,0.08)] space-y-4">
      ...
      <div>
        <label htmlFor="bkname" className="font-label-md text-label-md text-on-surface-variant block mb-1.5">
        ...
        <input
          ...
          placeholder={t('gameBaskin31.namePlaceholder')}
          onKeyDown={(e) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <GameInfoPanel
        description={t('gameBaskin31.infoDescription')}
        steps={t('gameBaskin31.infoSteps', { returnObjects: true }) as string[]}
      />
            ...
            {isStaff && (
              <div className="bg-surface-container-lowest rounded-xl p-5 shadow-[0_4px_20px_rgba(39,101,168,0.08)]">
                <div className="flex items-center justify-between mb-2">
                  <h4 className="font-title-md text-title-md text-on-surface">{t('gameBaskin31.settingsTitle')}</h4>
                  <button
