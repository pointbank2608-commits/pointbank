...
function defaultItems(): GameItem[] {
  return Array.from({ length: 12 }, (_, i) => i + 1).map((n) => ({
    id: uid(),
    label: i18n.t('gameTwoDice.defaultItem', { n }),
  }));
}

export default function TwoDicePage() {
  const { t } = useTranslation();
  const g = useGameTemplates({ gameType: 'twodice', defaultItems });
  const {
    isStaff,
    classes,
    staffClassId,
  ...
  async function addQuickItem() {
    ...
    if (await persistItems([...selected.items, { id: uid(), label: i18n.t('gameTwoDice.defaultItem', { n }) }])) {
  ...
  async function clearAllItems() {
    ...
    if (!confirm(t('gameTwoDice.clearAllConfirm'))) return;
    await persistItems([]);
  }
  ...
  const classPicker = isStaff ? (
    ...
    <h2 className="font-title-md text-title-md text-on-surface">
      {t('gameTwoDice.studentClassTitle', { className: studentClassName })}
    </h2>
  );
  ...
  const createForm = isStaff && showCreateForm && (
    <div className="bg-surface-container-lowest rounded-xl p-5 shadow-[0_4px_20px_rgba(39,101,168,0.08)] space-y-4">
      ...
      <div>
        <label htmlFor="tdname" className="font-label-md text-label-md text-on-surface-variant block mb-1.5">
        ...
        <input
          ...
          placeholder={t('gameTwoDice.namePlaceholder')}
          onKeyDown={(e) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <GameInfoPanel
        ...
        steps={t('gameTwoDice.infoSteps', { returnObjects: true }) as string[]}
      />
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          ...
          <GameThemeFrame gameType="twodice"
            roster={roster}
            ...
            {isStaff && (
              <div className="bg-surface-container-lowest rounded-xl p-5 shadow-[0_4px_20px_rgba(39,101,168,0.08)]">
                <div className="flex items-center justify-between mb-2">
                  <h4 className="font-title-md text-title-md text-on-surface">{t('gameTwoDice.settingsTitle')}</h4>
                  <button
                ...
                {editorOpen && (
                  <div className="space-y-1 divide-y divide-surface-container">
                    ...
                    <div className="pt-3">
                      <OpenInOtherGame currentType="twodice" itemCount={selected.items.length} onOpen={openInOtherGame} />
