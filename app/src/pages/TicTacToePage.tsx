...
const TICTACTOE_PLACE_SOUND: MusicSelection = {
  ...
  path: '',
  name: '틱택토 말 놓는 소리',
  url: '/sounds/tictactoe-place.wav?v=1',
};
...
function defaultItems(): GameItem[] {
  return [1, 2, 3, 4, 5, 6, 7, 8, 9].map((n) => ({ id: uid(), label: i18n.t('gameTicTacToe.defaultItem', { n }) }));
}

export default function TicTacToePage() {
  const { t } = useTranslation();
  const g = useGameTemplates({ gameType: 'tictactoe', defaultItems });
  const {
    isStaff,
    classes,
    staffClassId,
  ...
  async function addQuickItem() {
    ...
    if (await persistItems([...selected.items, { id: uid(), label: i18n.t('gameTicTacToe.defaultItem', { n }) }])) {
  ...
  const classPicker = isStaff ? (
    ...
    <h2 className="font-title-md text-title-md text-on-surface">
      {t('gameTicTacToe.studentClassTitle', { className: studentClassName })}
    </h2>
  );
  ...
  const createForm = isStaff && showCreateForm && (
    <div className="bg-surface-container-lowest rounded-xl p-5 shadow-[0_4px_20px_rgba(39,101,168,0.08)] space-y-4">
      ...
      <div>
        <label htmlFor="ttname" className="font-label-md text-label-md text-on-surface-variant block mb-1.5">
        ...
        <input
          ...
          placeholder={t('gameTicTacToe.namePlaceholder')}
          onKeyDown={(e) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <GameInfoPanel
        ...
        steps={t('gameTicTacToe.infoSteps', { returnObjects: true }) as string[]}
      />
            ...
            {isStaff && (
              <div className="bg-surface-container-lowest rounded-xl p-5 shadow-[0_4px_20px_rgba(39,101,168,0.08)]">
                <div className="flex items-center justify-between mb-2">
                  <h4 className="font-title-md text-title-md text-on-surface">{t('gameTicTacToe.settingsTitle')}</h4>
                  <button
