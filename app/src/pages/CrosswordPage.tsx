...
function defaultItems(): GameItem[] {
  return [1, 2, 3, 4, 5].map((n) => ({ id: uid(), label: i18n.t('gameCrossword.defaultItem', { n }) }));
}

export default function CrosswordPage() {
  const { t } = useTranslation();
  const g = useGameTemplates({ gameType: 'crossword', defaultItems });
  const {
    isStaff,
    classes,
    staffClassId,
  ...
  async function addQuickItem() {
    ...
    if (await persistItems([...selected.items, { id: uid(), label: i18n.t('gameCrossword.defaultItem', { n }) }])) {
  ...
  async function clearAllItems() {
    ...
    if (!confirm(t('gameCrossword.clearAllConfirm'))) return;
    await persistItems([]);
  }

  async function handleStyleChange(style: 'board' | 'blocks') {
    if (!selected) return;
    await persistConfig({ ...selected.config, crosswordStyle: style });
  }
  ...
  const classPicker = isStaff ? (
    ...
    <h2 className="font-title-md text-title-md text-on-surface">
      {t('gameCrossword.studentClassTitle', { className: studentClassName })}
    </h2>
  );
  ...
  const createForm = isStaff && showCreateForm && (
    <div className="bg-surface-container-lowest rounded-xl p-5 shadow-[0_4px_20px_rgba(39,101,168,0.08)] space-y-4">
      ...
      <div>
        <label htmlFor="cwname" className="font-label-md text-label-md text-on-surface-variant block mb-1.5">
        ...
        <input
          ...
          placeholder={t('gameCrossword.namePlaceholder')}
          onKeyDown={(e) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <GameInfoPanel
        description={t('gameCrossword.infoDescription')}
        steps={t('gameCrossword.infoSteps', { returnObjects: true }) as string[]}
      />
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          {classPicker}
          <div>
            <GameThemeFrame gameType="crossword" roster={roster} className="bg-[#fffdf8] rounded-[28px] p-4 md:p-6 shadow-[0_8px_28px_rgba(0,107,93,0.08)]">
        ...
        <div className="space-y-6">
          ...
          <GameThemeFrame gameType="crossword"
            roster={roster}
          ...
          >
            <Crossword
              ...
              boardStyle={selected.config.crosswordStyle === 'blocks' ? 'blocks' : 'board'}
              editable={isStaff}
            ...
            {isStaff && (
              <div className="bg-surface-container-lowest rounded-xl p-5 shadow-[0_4px_20px_rgba(39,101,168,0.08)]">
                <div className="flex items-center justify-between mb-2">
                  <h4 className="font-title-md text-title-md text-on-surface">{t('gameCrossword.settingsTitle')}</h4>
                  <button
                ...
                <div className="flex flex-wrap items-center gap-2 py-2">
                  <span className="font-label-md text-label-md text-on-surface-variant shrink-0">
                    {t('gameCrossword.styleLabel')}
                  </span>
                  {(['board', 'blocks'] as const).map((style) => {
                    const on = (selected.config.crosswordStyle ?? 'board') === style;
                    return (
                      <button
                        key={style}
                        type="button"
                      ...
                      >
                        {style === 'board' ? t('gameCrossword.styleBoard') : t('gameCrossword.styleBlocks')}
                      </button>
                    );
                  })}
                </div>
                ...
                {editorOpen && (
                  <div className="space-y-1 divide-y divide-surface-container">
                    ...
                    <div className="pt-3">
                      <OpenInOtherGame currentType="crossword" itemCount={selected.items.length} onOpen={openInOtherGame} />
