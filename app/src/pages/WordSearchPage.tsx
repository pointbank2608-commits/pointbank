...
function defaultItems(): GameItem[] {
  return [1, 2, 3, 4, 5].map((n) => ({ id: uid(), label: i18n.t('gameWordSearch.defaultItem', { n }) }));
}

export default function WordSearchPage() {
  const { t } = useTranslation();
  const g = useGameTemplates({ gameType: 'wordsearch', defaultItems });
  const {
    isStaff,
    classes,
    staffClassId,
  ...
  async function addQuickItem() {
    ...
    if (await persistItems([...selected.items, { id: uid(), label: i18n.t('gameWordSearch.defaultItem', { n }) }])) {
  ...
  async function clearAllItems() {
    ...
    if (!confirm(t('gameWordSearch.clearAllConfirm'))) return;
    await persistItems([]);
  }
  ...
  const classPicker = isStaff ? (
    ...
    <h2 className="font-title-md text-title-md text-on-surface">
      {t('gameWordSearch.studentClassTitle', { className: studentClassName })}
    </h2>
  );
  ...
  const createForm = isStaff && showCreateForm && (
    <div className="bg-surface-container-lowest rounded-xl p-5 shadow-[0_4px_20px_rgba(39,101,168,0.08)] space-y-4">
      ...
      <div>
        <label htmlFor="wsname" className="font-label-md text-label-md text-on-surface-variant block mb-1.5">
        ...
        <input
          ...
          placeholder={t('gameWordSearch.namePlaceholder')}
          onKeyDown={(e) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <GameInfoPanel
        description={t('gameWordSearch.infoDescription')}
        steps={t('gameWordSearch.infoSteps', { returnObjects: true }) as string[]}
      />
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          ...
          <div>
            <GameThemeFrame gameType="wordsearch" roster={roster} className="bg-[#fffdf8] rounded-[28px] p-4 md:p-6 shadow-[0_8px_28px_rgba(0,107,93,0.08)]">
        ...
        <div className="space-y-6">
          ...
          <GameThemeFrame gameType="wordsearch"
            roster={roster}
            ...
            {isStaff && (
              <div className="bg-surface-container-lowest rounded-xl p-5 shadow-[0_4px_20px_rgba(39,101,168,0.08)]">
                <div className="flex items-center justify-between mb-2">
                  <h4 className="font-title-md text-title-md text-on-surface">{t('gameWordSearch.settingsTitle')}</h4>
                  <button
