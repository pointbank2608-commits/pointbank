...
function defaultItems(): GameItem[] {
  return [1, 2, 3, 4, 5].map((n) => ({ id: uid(), label: i18n.t('gamePassBall.defaultItem', { n }) }));
}

const DEFAULT_RANGE = { min: 10, max: 30 };

export default function PassBallPage() {
  const { t } = useTranslation();
  const g = useGameTemplates({
    gameType: 'passball',
    defaultItems,
    defaultConfig: () => ({ ballRange: DEFAULT_RANGE }),
  });
  const {
    isStaff,
    academy,
  ...
  async function addQuickItem() {
    ...
    if (await persistItems([...selected.items, { id: uid(), label: i18n.t('gamePassBall.defaultItem', { n }) }])) {
  ...
  async function clearAllParticipants() {
    ...
    if (!confirm(t('gamePassBall.clearAllConfirm'))) return;
    await persistItems([]);
  }
  ...
  async function commitRange(nextRange: { min: number; max: number }) {
    ...
    const nextConfig = { ...selected.config, ballRange: { min, max } };
  ...
  const classPicker = isStaff ? (
    ...
    <h2 className="font-title-md text-title-md text-on-surface">
      {t('gamePassBall.studentClassTitle', { className: studentClassName })}
    </h2>
  );
  ...
  const createForm = isStaff && showCreateForm && (
    <div className="bg-surface-container-lowest rounded-xl p-5 shadow-[0_4px_20px_rgba(39,101,168,0.08)] space-y-4">
      ...
      <div>
        <label htmlFor="pbname" className="font-label-md text-label-md text-on-surface-variant block mb-1.5">
        ...
        <input
          ...
          placeholder={t('gamePassBall.namePlaceholder')}
          onKeyDown={(e) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <GameInfoPanel
        description={t('gamePassBall.infoDescription')}
        steps={t('gamePassBall.infoSteps', { returnObjects: true }) as string[]}
      />
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <div className="mt-3 text-center font-body-md text-body-md text-on-surface-variant">
              {isStaff ? t('gamePassBall.emptyStaff') : t('gamePassBall.emptyStudent')}
            </div>
          </div>
          {templateRow}
          {createForm}
            ...
            {isStaff && (
              <div className="bg-surface-container-lowest rounded-xl p-5 shadow-[0_4px_20px_rgba(39,101,168,0.08)]">
                <div className="flex items-center justify-between mb-2">
                  <h4 className="font-title-md text-title-md text-on-surface">{t('gamePassBall.settingsTitle')}</h4>
                  <button
                    ...
                    <div className="pt-3">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <input
                          ...
                          className="w-[70px] bg-surface-container-low border border-outline-variant rounded-lg px-2 py-1.5 font-body-md text-sm text-on-surface text-center focus:border-primary focus:ring-1 focus:ring-primary outline-none"
                        />
                        <span className="font-body-md text-body-md text-on-surface-variant">{t('gamePassBall.seconds')}</span>
                      </div>
                    </div>

                    {academy && (
                      <div className="pt-3 divide-y divide-surface-container">
                        <GameMusicPicker
                        ...
                        <GameMusicPicker
                          academyId={academy.id}
                          isStaff={isStaff}
                          label={t('gamePassBall.resultSoundLabel')}
