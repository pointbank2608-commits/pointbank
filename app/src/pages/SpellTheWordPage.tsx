...
function defaultItems(): GameItem[] {
  return [1, 2, 3, 4, 5].map((n) => ({ id: uid(), label: i18n.t('gameSpellWord.defaultItem', { n }) }));
}

const DEFAULT_PREVIEW_SECONDS = 3;

export default function SpellTheWordPage() {
  const { t } = useTranslation();
  const g = useGameTemplates({
    gameType: 'spellword',
    defaultItems,
    defaultConfig: () => ({ spellPreviewSeconds: DEFAULT_PREVIEW_SECONDS }),
  });
  const {
    isStaff,
    classes,
  ...
  useEffect(() => {
    setPreviewInput(String(previewSeconds));
  }, [previewSeconds]);
  ...
  async function clearAllItems() {
    ...
    if (!confirm(t('gameSpellWord.clearAllConfirm'))) return;
    await persistItems([]);
  }
  ...
  const classPicker = isStaff ? (
    ...
    <h2 className="font-title-md text-title-md text-on-surface">
      {t('gameSpellWord.studentClassTitle', { className: studentClassName })}
    </h2>
  );
  ...
  const createForm = isStaff && showCreateForm && (
    <div className="bg-surface-container-lowest rounded-xl p-5 shadow-[0_4px_20px_rgba(39,101,168,0.08)] space-y-4">
      <div>
        <label htmlFor="swname" className="font-label-md text-label-md text-on-surface-variant block mb-1.5">
        ...
        <input
          ...
          placeholder={t('gameSpellWord.namePlaceholder')}
          onKeyDown={(e) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <GameInfoPanel
        description={t('gameSpellWord.infoDescription')}
        steps={t('gameSpellWord.infoSteps', { returnObjects: true }) as string[]}
      />
