...
function defaultItems(): GameItem[] {
  return [1, 2, 3, 4, 5].map((n) => ({ id: uid(), label: i18n.t('gameAirplane.defaultItem', { n }) }));
}

export default function AirplanePage() {
  const { t } = useTranslation();
  const g = useGameTemplates({ gameType: 'airplane', defaultItems });
  const {
    isStaff,
    classes,
    staffClassId,
  ...
  async function clearAllItems() {
    ...
    if (!confirm(t('gameAirplane.clearAllConfirm'))) return;
    await persistItems([]);
  }
  ...
  async function addQuickItem() {
    ...
    if (await persistItems([...selected.items, { id: uid(), label: i18n.t('gameAirplane.defaultItem', { n }) }])) {
  ...
  async function handleStyleChange(style: 'wood' | 'clay') {
    if (!selected) return;
    await persistConfig({ ...selected.config, airplaneStyle: style });
  }
  ...
  const classPicker = isStaff ? (
    ...
    <h2 className="font-title-md text-title-md text-on-surface">
      {t('gameAirplane.studentClassTitle', { className: studentClassName })}
    </h2>
  );
  ...
  const createForm = isStaff && showCreateForm && (
    <div className="bg-surface-container-lowest rounded-xl p-5 shadow-[0_4px_20px_rgba(39,101,168,0.08)] space-y-4">
      ...
      <div>
        <label htmlFor="apname" className="font-label-md text-label-md text-on-surface-variant block mb-1.5">
        ...
        <input
          ...
          placeholder={t('gameAirplane.namePlaceholder')}
          onKeyDown={(e) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <GameInfoPanel
        description={t('gameAirplane.infoDescription')}
        steps={t('gameAirplane.infoSteps', { returnObjects: true }) as string[]}
      />
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          ...
          <div>
            <GameThemeFrame gameType="airplane" roster={roster} className="bg-[#fffdf8] rounded-[28px] p-4 md:p-6 shadow-[0_8px_28px_rgba(0,107,93,0.08)]">
        ...
        <div className="space-y-6">
          ...
          <GameThemeFrame gameType="airplane"
            roster={roster}
          ...
          >
            <Airplane
              ...
              boardStyle={selected.config.airplaneStyle === 'clay' ? 'clay' : 'wood'}
              editable={isStaff}
            ...
            {isStaff && (
              <div className="bg-surface-container-lowest rounded-xl p-5 shadow-[0_4px_20px_rgba(39,101,168,0.08)]">
                <div className="flex items-center justify-between mb-2">
                  <h4 className="font-title-md text-title-md text-on-surface">{t('gameAirplane.settingsTitle')}</h4>
                  <button
                ...
                <div className="flex flex-wrap items-center gap-2 py-2">
                  <span className="font-label-md text-label-md text-on-surface-variant shrink-0">
                    {t('gameAirplane.styleLabel')}
                  </span>
                  {(['wood', 'clay'] as const).map((style) => {
                    const on = (selected.config.airplaneStyle ?? 'wood') === style;
                    return (
                      <button
                        key={style}
                        type="button"
                      ...
                      >
                        {style === 'wood' ? t('gameAirplane.styleWood') : t('gameAirplane.styleClay')}
                      </button>
                    );
                  })}
                </div>
                ...
                {editorOpen && (
                  <div>
                    ...
                    <div className="mt-3 pt-3 border-t border-surface-container">
                      <OpenInOtherGame currentType="airplane" itemCount={selected.items.length} onOpen={openInOtherGame} />
