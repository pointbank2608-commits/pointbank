...
const DEFAULT_START_SCORE = 100;
const DEFAULT_BET_OPTIONS = [10, 20, 50];

export default function WinLoseQuizPage() {
  const { t } = useTranslation();
  const g = useGameTemplates({
    gameType: 'winlosequiz',
    defaultItems,
  ...
  const startScore = selected?.config.winLoseStartScore ?? DEFAULT_START_SCORE;
  const betOptions = selected?.config.winLoseBetOptions ?? DEFAULT_BET_OPTIONS;
  ...
  function setStartScore(n: number) {
    void persistConfig({ ...selected?.config, winLoseStartScore: Math.max(0, n || DEFAULT_START_SCORE) });
  }

  function setBetOption(index: number, value: number) {
  ...
  const classPicker = isStaff ? (
    ...
    <h2 className="font-title-md text-title-md text-on-surface">
      {t('gameWinLoseQuiz.studentClassTitle', { className: studentClassName })}
    </h2>
  );
    ...
    <div className="bg-surface-container-lowest rounded-xl p-5 shadow-[0_4px_20px_rgba(39,101,168,0.08)] space-y-4">
      <div>
        ...
        <input
          ...
          placeholder={t('gameWinLoseQuiz.namePlaceholder')}
          onKeyDown={(e) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <GameInfoPanel
        description={t('gameWinLoseQuiz.infoDescription')}
        steps={t('gameWinLoseQuiz.infoSteps', { returnObjects: true }) as string[]}
      />
            ...
            {isStaff && (
              <div className="bg-surface-container-lowest rounded-xl p-5 shadow-[0_4px_20px_rgba(39,101,168,0.08)]">
                <div className="flex items-center justify-between mb-2">
                  <h4 className="font-title-md text-title-md text-on-surface">{t('gameWinLoseQuiz.settingsTitle')}</h4>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex flex-wrap items-center gap-4">
                      <div className="flex items-center gap-2">
                        <label htmlFor="wlqstart" className="font-label-md text-label-md text-on-surface-variant shrink-0">
                          {t('gameWinLoseQuiz.startScoreLabel')}
                        </label>
                        <input
                          id="wlqstart"
                          type="number"
                          ...
                          className="w-24 bg-surface-container-lowest border border-outline-variant rounded-lg px-3 py-1.5 font-body-md text-sm text-on-surface focus:border-primary focus:ring-1 focus:ring-primary outline-none"
                        />
                      </div>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      <span className="font-label-md text-label-md text-on-surface-variant shrink-0">
                        {t('gameWinLoseQuiz.betOptionsLabel')}
                      </span>
                      {betOptions.map((amount, i) => (
                        <input
                          ...
                          min={1}
                          value={amount}
                          onChange={(e) => setBetOption(i, Number(e.target.value))}
                      ...
                      <div key={q.id} className="bg-surface-container-low rounded-lg p-4 space-y-2">
                        <div className="flex items-center justify-between">
                          <label className="font-label-md text-label-md text-on-surface-variant">
                            {t('gameWinLoseQuiz.questionLabel', { n: qi + 1 })}
                          </label>
                          <button
                            type="button"
                          ...
                          >
                            {t('gameWinLoseQuiz.removeQuestionButton')}
                          </button>
                        </div>
                        <input
                          value={q.question}
                          ...
                          onBlur={commitOnBlur}
                          placeholder={t('gameWinLoseQuiz.questionPlaceholder')}
                        ...
                        <div className="font-caption text-caption text-on-surface-variant">
                          {t('gameWinLoseQuiz.markCorrectHint')}
                        </div>
                          ...
                          {q.choices.map((choice, ci) => (
                            <div key={ci} className="flex items-center gap-2">
                              <button
                                ...
                                aria-label={t('gameWinLoseQuiz.markCorrectHint')}
                              ...
                              <input
                                ...
                                onBlur={commitOnBlur}
                                placeholder={t('gameWinLoseQuiz.choicePlaceholder', { n: ci + 1 })}
                        ...
                        {q.choices.length < 6 && (
                          ...
                          >
                            {t('gameWinLoseQuiz.addChoiceButton')}
                          </button>
                        )}
                      </div>
                    ))}
