...
function defaultItems(): GameItem[] {
  return [1, 2, 3, 4, 5, 6].map((n) => ({ id: uid(), label: i18n.t('gamePopcorn.defaultItem', { n }) }));
}

export default function PopcornPage() {
  const { t } = useTranslation();
  const g = useGameTemplates({ gameType: 'popcorn', defaultItems });
  const {
    isStaff,
    classes,
    staffClassId,
  ...
  const classPicker = isStaff ? (
    ...
    <h2 className="font-title-md text-title-md text-on-surface">
      {t('gamePopcorn.studentClassTitle', { className: studentClassName })}
    </h2>
  );
  ...
  const createForm = isStaff && showCreateForm && (
    <div className="bg-surface-container-lowest rounded-xl p-5 shadow-[0_4px_20px_rgba(39,101,168,0.08)] space-y-4">
      ...
      <div>
        <label htmlFor="pcname" className="font-label-md text-label-md text-on-surface-variant block mb-1.5">
        ...
        <input
          ...
          placeholder={t('gamePopcorn.namePlaceholder')}
          onKeyDown={(e) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <GameInfoPanel
        description={t('gamePopcorn.infoDescription')}
        steps={t('gamePopcorn.infoSteps', { returnObjects: true }) as string[]}
      />
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          ...
          <div>
            <GameThemeFrame gameType="popcorn" roster={roster} className="bg-[#fffdf8] rounded-[28px] p-4 md:p-6 shadow-[0_8px_28px_rgba(0,107,93,0.08)]">
              <Popcorn items={demoItems} />
            </GameThemeFrame>
            <div className="mt-3 text-center font-body-md text-body-md text-on-surface-variant">
              {isStaff ? t('gamePopcorn.emptyStaff') : t('gamePopcorn.emptyStudent')}
            </div>
          </div>
        ...
        <div className="space-y-6">
          ...
          <GameThemeFrame gameType="popcorn"
            roster={roster}
