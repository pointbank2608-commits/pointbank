...
function defaultItems(): GameItem[] {
  return [1, 2, 3, 4, 5].map((n) => ({ id: uid(), label: i18n.t('gameAnagram.defaultItem', { n }) }));
}

export default function AnagramPage() {
  const { t } = useTranslation();
  const g = useGameTemplates({ gameType: 'anagram', defaultItems });
  const {
    isStaff,
    classes,
    staffClassId,
  ...
  async function clearAllItems() {
    ...
    if (!confirm(t('gameAnagram.clearAllConfirm'))) return;
    await persistItems([]);
  }
  ...
  async function addQuickItem() {
    ...
    if (await persistItems([...selected.items, { id: uid(), label: i18n.t('gameAnagram.defaultItem', { n }) }])) {
  ...
  const classPicker = isStaff ? (
    ...
    <h2 className="font-title-md text-title-md text-on-surface">
      {t('gameAnagram.studentClassTitle', { className: studentClassName })}
    </h2>
  );
  ...
  const createForm = isStaff && showCreateForm && (
    <div className="bg-surface-container-lowest rounded-xl p-5 shadow-[0_4px_20px_rgba(39,101,168,0.08)] space-y-4">
      ...
      <div>
        <label htmlFor="agname" className="font-label-md text-label-md text-on-surface-variant block mb-1.5">
        ...
        <input
          ...
          placeholder={t('gameAnagram.namePlaceholder')}
          onKeyDown={(e) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <GameInfoPanel
        description={t('gameAnagram.infoDescription')}
        steps={t('gameAnagram.infoSteps', { returnObjects: true }) as string[]}
      />
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          ...
          <div>
            <GameThemeFrame gameType="anagram" roster={roster} className="bg-[#fffdf8] rounded-[28px] p-4 md:p-6 shadow-[0_8px_28px_rgba(0,107,93,0.08)]">
              <Anagram items={demoItems} />
            </GameThemeFrame>
        ...
        <div className="space-y-6">
          ...
          <GameThemeFrame gameType="anagram"
            roster={roster}
            ...
            {isStaff && (
              <div className="bg-surface-container-lowest rounded-xl p-5 shadow-[0_4px_20px_rgba(39,101,168,0.08)]">
                <div className="flex items-center justify-between mb-2">
                  <h4 className="font-title-md text-title-md text-on-surface">{t('gameAnagram.settingsTitle')}</h4>
                  <button
